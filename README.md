...
You can use block to extend a JSON value which will be generated by partial.

```ruby
json.partial! 'posts/post', post: post do
  json.with_star current_user.stared_post.where(id: post.id).exists?
end
```

Collection can be used, but `:as` and `:locals` options do not work.
You can access the member object via block parameter.

```ruby
json.partial! 'posts/post', collection: @posts, as: :post do |post|
  json.comments post.filtered_comments_for_user(current_user)
end
```
